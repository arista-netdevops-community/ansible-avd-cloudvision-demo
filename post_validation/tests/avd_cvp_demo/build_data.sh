...
echo "1.2 Building intended BGP sessions."
...
echo "1.3 Building intended environment sessions."
...
echo "1.4 Building intended port-channel status."
...
echo "1.5 Building intended MLAG status."
python3 ${AVD_PYMOD}/intended/mlag.py -src=${STRUCTURED_CONFIGS_DIR} -dst=${INTENDED_DATA_DIR}/mlag.yml -sfm string -dfm yaml --case from_struct_config
...
echo "2.2 Building current BGP sessions."
...
echo "2.3 Building current environment sessions."
...
echo "2.4 Building current port-channel status."
...
echo "2.5 Building current MLAG status."
python3 ${AVD_PYMOD}/current/mlag.py -src=${DUT_STATE_JSON} -dst=${CURRENT_DATA_DIR}/mlag.yml --src_format json --dst_format yaml --case mlag_status_from_dut
